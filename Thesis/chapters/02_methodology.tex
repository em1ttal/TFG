...
We configured our TabNet model with the following optimized hyperparameters to ensure sufficient capacity for the Adult Income dataset:
\begin{itemize}
    \item \textbf{Decision/Attention Width ($N_d, N_a$)}: 64 (increased from default to capture complex patterns)
    \item \textbf{Decision Steps ($N_{steps}$)}: 5 (increased depth for better reasoning)
    \item \textbf{Feature Reusage ($\gamma$)}: 1.5 (encourages feature sharing across steps)
    \item \textbf{Sparsity ($\lambda_{sparse}$)}: $1e-3$ (reduced regularization to allow using more features)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Challenges}: Unlike the clean 2D spirals, this dataset features mixed data types, class imbalance (76\% vs 24\%), and higher dimensionality.
    \item \textbf{Enhancements}: To address these challenges, we implemented:
    \begin{itemize}
        \item \textbf{Feature Engineering}: Created derived features such as \textit{age\_group}, \textit{capital\_change}, \textit{is\_married}, and \textit{age\_education} interactions to improve predictive power.
        \item \textbf{Class Balancing}: Applied computed class weights ({0: 0.66, 1: 2.09}) during training to prevent the model from biasing towards the majority class.
    \end{itemize}
\end{itemize}
