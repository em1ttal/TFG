...
Traditionally, the "right to be forgotten"—enshrined in regulations like the General Data Protection Regulation (GDPR) in the European Union—has been applied to databases. If you ask a company to delete your data, they remove the row from their SQL table. Problem solved, right? Not exactly. 

When a machine learning model is trained on that data, the information doesn't just sit in a database row; it gets baked into the model's parameters (weights and biases). Even after the original data is deleted from the database, the model might still "remember" the information, potentially leaking it through its predictions or being vulnerable to membership inference attacks. 

This leads us to the concept of \textbf{Machine Unlearning}. The goal is simple yet technically challenging: we want to make the model "forget" specific data points, classes, or subsets of data, such that the resulting model behaves as if it had \emph{never seen that data in the first place}.
...
We utilize \textbf{TabNet}, a state-of-the-art deep learning architecture designed for tabular data, which combines the interpretability of decision trees with the learning capacity of neural networks.

Our main objectives are:
\begin{enumerate}
    \item To implement a robust tabular classification pipeline using TabNet on a complex, non-linear dataset (the Spiral Dataset).
    \item To simulate real-world "forget requests," such as removing an entire class or specific subsets of data.
    \item To implement and evaluate five distinct noise-based unlearning strategies:
    \begin{itemize}
        \item Gaussian Noise Injection
        \item Laplacian Noise Injection
        \item Adaptive Noise Injection
        \item Layer-wise Noise Injection
        \item Gradient-based Unlearning (Gradient Ascent)
    \end{itemize}
